...
class OpenaiService
  ...
  def generate_story_mbti_question(dimension, question_number, story_mode)
    ...
    story_settings = {
      'horror' => {
        atmosphere: 'ホラー・スリラー',
        setting: '月明かりも届かない鬱蒼とした森の奥にある、埃をかぶった古い洋館。' \
                 'きしむ床板、壁に飾られた不気味な肖像画、そしてどこからともなく聞こえるささやき声。' \
                 'その屋敷の地下室には、決して開けてはならないと伝えられる鉄の扉がひっそりと隠されている。',
        tone: '息をひそめるような静けさの中に潜む、張り詰めた緊張感と得体の知れない恐怖。一瞬の油断も許されない、精神的に追い詰められる状況。主人公は、理性を失う寸前まで追い込まれながらも、恐怖の根源に迫っていく。'
      },
      'adventure' => {
        atmosphere: 'アドベンチャー・冒険',
        setting: '古代文明の遺跡が眠る、地図にもない秘境のジャングル。' \
                 '巨大な滝の裏に隠された洞窟、罠が仕掛けられた神殿、そして絶滅したはずの生物が棲息する。' \
                 'その奥には、世界の歴史を書き換えるほどの力を秘めた伝説の秘宝が眠っている。',
        tone: '未知なるものを発見する高揚感と、常に隣り合わせの危険。困難を乗り越える勇気と、仲間との絆が試されるエキサイティングな状況。主人公は、失われた知識と技術を巡る壮大な冒険の中で、自らの限界に挑む。'
      },
      'mystery' => {
        atmosphere: 'ミステリー・推理',
        setting: '大雪で孤立した山荘。通信手段は断たれ、閉ざされた空間で次々と起こる不可解な殺人事件。' \
                 '容疑者は限られた人数、そしてそれぞれの心に秘められた嘘と動機。' \
                 '山荘に住む者たちの過去の因縁が、複雑に絡み合いながら事件の真相を隠蔽する。',
        ...
        messages: [
          {
            role: 'system',
            content: 'あなたは心理学者で、性格分析の専門家です。また、物語作家でもあります。' \
                     '指定された物語モードの世界観に沿って、主人公の性格や行動パターンを測る質問を作成してください。' \
                     '必ず有効なJSON形式で回答し、他のテキストは含めないでください。'
          },
          {
            role: 'user',
